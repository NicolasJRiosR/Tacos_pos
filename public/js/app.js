...
function agregarProducto(id, nombre, precio, codigo_barra) {
  let existente = carrito.find((p) => p.id === id);

  if (!existente) {
    carrito.push({ id, nombre, precio, cantidad: 1, codigo_barra });
    renderTicket(id); // animación solo si es nuevo
  } else {
    existente.cantidad++;
    renderTicket(); // actualización normal
  }
}
...
function cobrar() {
  ...
  // Enviar carrito al servidor para descontar stock
  fetch("index.php?controller=Producto&action=actualizarStock", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(carrito), // enviamos todo el carrito con codigo_barra
  })
    .then((res) => res.json())
    .then((data) => {
      if (data.success) {
