document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("index.php?controller=Producto&action=guardar", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          tr.dataset.originalNombre = data.nombre;
          tr.dataset.originalPrecio = data.precio;
          tr.dataset.originalCodigo = data.codigo_barra;
          ...
          // Efecto visual opcional para notar la fila nueva
  ...
  tabla.addEventListener("click", function (e) {
    ...
    if (e.target.classList.contains("editar")) {
      ...
      precio.innerHTML = `<input type="number" step="0.01" value="${precio.textContent
        .replace("$", "")
        .trim()}">`;
      ...
      fetch("index.php?controller=Producto&action=actualizar", {
        ...
        .then((data) => {
          if (data.success) {
            ...
            // Actualizar data-attributes con nuevos valores
            tr.dataset.originalNombre = nombre;
            tr.dataset.originalPrecio = precio;
            tr.dataset.originalCodigo = codigo;
